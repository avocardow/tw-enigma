...
function addLicenseHeader(content) {
  ...
  const contentWithoutShebang = shebang ? content.replace(/^#!.*\n/, '') : content;
...
function main() {
    ...
    if (dryRun) {
      ...
      switch (result.status) {
        case 'added':
          ...
          break;
        case 'skipped':
          if (verbose) console.log(`⏭️  SKIP: ${relativePath} (${result.reason})`);
          results.skipped++;
          break;
        case 'error':
  ...
  console.log('');
  console.log('📊 Summary:');
  console.log(`   ${dryRun ? 'Would add headers' : 'Headers added'}: ${results.added}`);
