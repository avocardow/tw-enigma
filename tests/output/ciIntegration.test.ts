...
import { CiIntegration, createCiIntegration } from '../../src/output/ciIntegration.js';
...
describe('CiIntegration', () => {
  ...
  describe('processReport', () => {
    ...
    it('should fail when major regressions detected', async () => {
      ...
      const scoreRegression = result.comparison?.regressions.find(r => r.type === 'score_decrease');
  ...
  describe('regression detection', () => {
    it('should detect performance score regression', async () => {
      const current: CssPerformanceReport = {
        ...
        recommendations: [], configuration: config, assets: [], chunkAnalysis: []
      };
    ...
    it('should detect size increase regression', async () => {
      const current: CssPerformanceReport = {
        ...
        metrics: {
          ...
          performanceScore: 0, totalOriginalSize: 0, totalOptimizedSize: 0,
          bundleCount: 0, totalChunkCount: 0, overallCompressionRatio: 0,
          totalCriticalCssSize: 0, averageLoadTime: 0, totalOptimizationTime: 0,
          bundles: []
        },
    ...
    it('should detect load time regression', async () => {
      const current: CssPerformanceReport = {
        ...
        metrics: {
          ...
          performanceScore: 0, totalOriginalSize: 0, totalOptimizedSize: 0,
          bundleCount: 0, totalChunkCount: 0, overallCompressionRatio: 0,
          totalCriticalCssSize: 0, totalCompressedSize: 0, totalOptimizationTime: 0,
          bundles: []
        },
        budgetAnalysis: { passed: true, violations: [], score: 100 },
        recommendations: [], configuration: config, assets: [], chunkAnalysis: []
      ...
      const loadTimeRegression = result.comparison?.regressions.find(r => r.type === 'load_time_increase');
    ...
    it('should detect performance improvements', async () => {
      const current: CssPerformanceReport = {
        ...
        metrics: {
          ...
          bundleCount: 0, totalChunkCount: 0,
          totalCriticalCssSize: 0, totalOptimizationTime: 0,
          bundles: []
        },
        budgetAnalysis: { passed: true, violations: [], score: 100 },
        recommendations: [], configuration: config, assets: [], chunkAnalysis: []
      ...
      const scoreImprovement = result.comparison?.improvements.find(i => i.type === 'score_increase');
      expect(scoreImprovement).toBeDefined();
      expect(scoreImprovement?.improvementPercent).toBe(10);
      
      // Should detect size reduction
      const sizeImprovement = result.comparison?.improvements.find(i => i.type === 'size_reduction');
      expect(sizeImprovement).toBeDefined();
      expect(sizeImprovement?.improvementPercent).toBe(20);
      
      // Should detect load time improvement
      const loadTimeImprovement = result.comparison?.improvements.find(i => i.type === 'load_time_improvement');
      expect(loadTimeImprovement).toBeDefined();
      
      // Should detect compression improvement
      const compressionImprovement = result.comparison?.improvements.find(i => i.type === 'compression_improvement');
