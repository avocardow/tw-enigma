...
import { CssReportGenerator, createCssReportGenerator } from '../../src/output/cssReportGenerator.js';
import type { CssOutputConfig, PerformanceBudget } from '../../src/output/cssOutputConfig.js';
...
describe('CssReportGenerator', () => {
  ...
  describe('generateReport', () => {
      ...
      const bundles: CssBundle[] = [
        {
          ...
          rules: [
            { selector: 'body', declarations: [{ property: 'margin', value: '0' }] }
          ],
          sourceMap: undefined
        },
        {
          ...
          rules: [
            { selector: '.vendor', declarations: [{ property: 'color', value: 'red' }] }
          ],
          sourceMap: undefined
        }
      ];
    ...
    it('should detect budget violations', async () => {
      ...
      const bundleSizeViolation = report.budgetAnalysis.violations.find(v => v.type === 'bundle_size');
      ...
      const criticalCssViolation = report.budgetAnalysis.violations.find(v => v.type === 'critical_css');
      ...
      expect(criticalCssViolation?.actual).toBeGreaterThan(criticalCssViolation?.limit || 0);
    ...
    it('should generate optimization recommendations', async () => {
      ...
      const chunkingRec = report.recommendations.find(r => r.category === 'chunking');
      ...
      const compressionRec = report.recommendations.find(r => r.category === 'compression');
      ...
      const criticalCssRec = report.recommendations.find(r => r.category === 'critical_css');
      ...
      const bundles: CssBundle[] = [
        {
          ...
          rules: [{ selector: 'body', declarations: [{ property: 'margin', value: '0' }] }],
          sourceMap: undefined
        }
      ];
  ...
  describe('exportReport', () => {
    ...
    it('should export report as Markdown', async () => {
      ...
      const markdownOutput = await generator.exportReport(mockReport, 'markdown');
    ...
    it('should throw error for unsupported format', async () => {
      const mockReport = {
        metadata: { timestamp: '', version: '', environment: '', configHash: '' },
        metrics: {
          totalOriginalSize: 0, totalOptimizedSize: 0, totalCompressedSize: 0,
          bundleCount: 0, totalChunkCount: 0, overallCompressionRatio: 0,
          totalCriticalCssSize: 0, averageLoadTime: 0, totalOptimizationTime: 0,
          performanceScore: 0, bundles: []
        },
      ...
      await expect(generator.exportReport(mockReport, 'xml' as any))
