...
describe('AtomicRollbackManager', () => {
  ...
  describe('Transaction Management', () => {
    ...
    it('should create transaction with description', () => {
      ...
      const transactionId = manager.beginTransaction(description);
  ...
  describe('Rollback Operations', () => {
    it('should rollback file creation', async () => {
      ...
      const fileExistsAfter = await fs.access(testFile).then(() => true, () => false);
    ...
    it('should rollback directory creation', async () => {
      ...
      const dirExists = await fs.access(testDirPath).then(() => true, () => false);
      ...
      const dirExistsAfter = await fs.access(testDirPath).then(() => true, () => false);
    ...
    it('should rollback multiple operations in reverse order', async () => {
      ...
      const file1Exists = await fs.access(file1).then(() => true, () => false);
      const file2Exists = await fs.access(file2).then(() => true, () => false);
  ...
  describe('Checkpoint Management', () => {
    ...
    it('should reject rollback to non-existent checkpoint', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain('Checkpoint non-existent-checkpoint not found');
    });
  });
