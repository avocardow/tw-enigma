...
describe('AtomicFileManager', () => {
  ...
  describe('Temporary File Creation', () => {
    ...
    it('should create temp directory if it does not exist', async () => {
      ...
      const dirExists = await fs.access(nonExistentDir).then(() => true, () => false);
    ...
    it('should support custom cleanup timeout', async () => {
      ...
      const tempInfo = await manager.createTempFile(targetPath, { cleanupTimeout: customTimeout });
  ...
  describe('Temporary File Cleanup', () => {
    it('should clean up specific temporary file', async () => {
      ...
      // Verify file exists
      const fileExists = await fs.access(tempInfo.path).then(() => true, () => false);
      ...
      const fileExistsAfter = await fs.access(tempInfo.path).then(() => true, () => false);
    ...
    it('should handle cleanup of non-existent file gracefully', async () => {
      ...
      await expect(manager.cleanupTempFile(tempInfo.operationId))
    ...
    it('should handle cleanup of unknown operation ID gracefully', async () => {
      await expect(manager.cleanupTempFile('unknown-operation-id'))
        .resolves.not.toThrow();
    });
  });
  ...
  describe('Temporary File Promotion', () => {
    it('should promote temporary file to final location', async () => {
      ...
      const tempExists = await fs.access(tempInfo.path).then(() => true, () => false);
  ...
  describe('Metrics Tracking', () => {
    it('should track operation metrics correctly', async () => {
      ...
      const tempInfo = await manager.createTempFile(path.join(testDir, 'target.txt'));
      await fs.writeFile(tempInfo.path, 'content');
      await manager.promoteTempFile(tempInfo.operationId);
