...
describe('HtmlExtractor', () => {
  ...
  describe('extractFromString', () => {
    it('should extract classes from simple HTML', async () => {
      const html = '<div class="container"><p class="text-blue-500">Hello</p></div>';
    ...
    it('should handle multiple classes on single element', async () => {
      const html = '<div class="flex justify-center items-center bg-red-500">Content</div>';
    ...
    it('should capture element context information', async () => {
      const html = '<div id="main" class="container" data-test="value">Content</div>';
    ...
    it('should handle case sensitivity option', async () => {
      ...
      const resultInsensitive = await insensitiveExtractor.extractFromString(html);
    ...
    it('should handle malformed HTML gracefully', async () => {
      const html = '<div class="test"><p class="broken">Unclosed<span class="nested">Content';
    ...
    it('should handle special characters in class names', async () => {
      const html = '<div class="class-with-dashes class_with_underscores class:with:colons class.with.dots">Content</div>';
    ...
    it('should handle elements without class attributes', async () => {
      const html = '<div><p>No classes here</p><span id="test">Still no classes</span></div>';
    ...
    it('should sanitize sensitive attributes', async () => {
      const html = '<div class="test" password="secret" token="abc123" id="safe" data-public="ok">Content</div>';
  ...
  describe('extractFromFile', () => {
    ...
    it('should reject files exceeding size limit', async () => {
      ...
      await expect(extractor.extractFromFile('/large.html'))
    ...
    it('should handle file read errors', async () => {
      ...
      await expect(extractor.extractFromFile('/nonexistent.html'))
        .rejects.toThrow(FileReadError);
    });
    ...
    it('should handle file stat errors', async () => {
      ...
      await expect(extractor.extractFromFile('/forbidden.html'))
        .rejects.toThrow(FileReadError);
    });
    ...
    it('should handle timeout for large files', async () => {
      ...
      mockFs.readFile.mockImplementation(() => 
        new Promise(resolve => setTimeout(() => resolve('<div>Content</div>'), 200))
      );
      
      await expect(shortTimeoutExtractor.extractFromFile('/slow.html'))
  ...
  describe('Error Handling', () => {
    it('should throw HtmlParsingError for invalid HTML processing', async () => {
      ...
      vi.spyOn(extractor as unknown as { parseClassAttribute: () => void }, 'parseClassAttribute').mockImplementation(() => {
        throw new Error('Mocked parsing error');
      });
      
      await expect(extractor.extractFromString(invalidHtml))
        .rejects.toThrow(HtmlParsingError);
    });
  ...
  describe('Convenience Functions', () => {
    ...
    it('should extract from HTML string with convenience function', async () => {
      const result = await extractClassesFromHtml('<div class="convenience">Test</div>');
    ...
    it('should extract from file with convenience function', async () => {
      mockFs.stat.mockResolvedValue({ size: 1000 } as Stats);
      mockFs.readFile.mockResolvedValue('<div class="file-convenience">Test</div>');
  ...
  describe('Edge Cases and Performance', () => {
    ...
    it('should handle Unicode and international characters in class names', async () => {
      const html = '<div class="测试-class класс-тест 日本語-クラス">Content</div>';
