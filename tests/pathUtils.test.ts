  ...
  describe('Path Validation', () => {
    describe('Basic Validation', () => {
      ...
      it('warns about whitespace in paths', () => {
        ...
        expect(result.isValid).toBe(true);
        expect(result.warnings).toContain('path has leading/trailing whitespace');
      });
    });
    ...
    describe('Security Validation', () => {
      it('detects path traversal attempts', () => {
        ...
        expect(result.warnings).toContain('path contains path traversal sequences');
      });
      ...
      it('detects null byte injection', () => {
        ...
        expect(result.isValid).toBe(false);
        expect(result.errors).toContain('path contains null bytes (security risk)');
      });

      it('detects suspicious characters', () => {
        const result = pathUtils.validatePath('/path/file<>.css');

        expect(result.warnings.some(w => w.includes('problematic characters'))).toBe(true);
      });
      ...
      it('can disable security checks', () => {
        ...
        expect(result.warnings).not.toContain('path contains path traversal sequences');
      });
    });
  ...
  describe('Path Normalization', () => {
    describe('Basic Normalization', () => {
      it('normalizes path separators', () => {
        const normalized = pathUtils.normalizePath('path\\with\\backslashes', true);
        expect(normalized).toBe('path/with/backslashes');
      });

      it('removes redundant separators', () => {
        const normalized = pathUtils.normalizePath('path//with///multiple////slashes');
    ...
    describe('Batch Operations', () => {
      it('processes multiple path calculations efficiently', () => {
        const pairs = [
          ...
          { from: '/project/src/contact.html', to: '/project/assets/contact.css' },
        ];
  ...
  describe('Caching System', () => {
    describe('Path Calculation Caching', () => {
      it('caches relative path calculations', () => {
        const result1 = pathUtils.calculateRelativePath('/from/path.html', '/to/path.css');
        const result2 = pathUtils.calculateRelativePath('/from/path.html', '/to/path.css');
  ...
  describe('Error Classes', () => {
    ...
    describe('PathSecurityError', () => {
      it('creates security-specific errors', () => {
        const error = new PathSecurityError('Security violation', '/dangerous/path');
    ...
    describe('PathValidationError', () => {
      it('creates validation-specific errors', () => {
        const error = new PathValidationError('Validation failed', '/invalid/path');
    ...
    describe('Real-World Path Scenarios', () => {
      it('handles typical web project structure', () => {
        const scenarios = [
          ...
          { from: '/project/index.html', to: '/project/assets/css/main.css', expected: 'assets/css/main.css' },
          // HTML in pages, CSS in root assets
          { from: '/project/pages/about.html', to: '/project/assets/styles.css', expected: '../assets/styles.css' },
          // Nested HTML, nested CSS
          { from: '/project/src/pages/deep/nested.html', to: '/project/src/assets/css/style.css', expected: '../../assets/css/style.css' },
          // Same directory
          { from: '/project/dist/index.html', to: '/project/dist/bundle.css', expected: 'bundle.css' },
        ];
    ...
    describe('Performance Characteristics', () => {
      it('performs well with repeated calculations', () => {
        ...
        for (let i = 0; i < iterations; i++) {
          pathUtils.calculateRelativePath(`/project/src/file${i}.html`, '/project/dist/common.css');
        }
      ...
      it('benefits from caching on repeated identical calculations', () => {
        ...
        // Run calculations
        for (let i = 0; i < iterations; i++) {
          pathUtils.calculateRelativePath('/project/src/index.html', '/project/dist/styles.css');
        }
