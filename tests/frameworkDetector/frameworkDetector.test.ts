...
describe('FrameworkDetector', () => {
  ...
  describe('Basic Framework Detection', () => {
    it('should detect React framework from package.json', async () => {
      ...
      await fs.writeFile(path.join(testDir, 'src', 'App.jsx'), 'export default function App() { return <div>Hello</div>; }');
    ...
    it('should detect Next.js framework from package.json and file structure', async () => {
      ...
      await fs.writeFile(path.join(testDir, 'pages', 'index.js'), 'export default function Home() { return <div>Home</div>; }');
...
export default defineConfig({
  plugins: [react()],
      ...
      await fs.writeFile(path.join(testDir, 'index.html'), '<!DOCTYPE html><html><head></head><body><div id="root"></div></body></html>');
    ...
    it('should handle projects with no framework detected', async () => {
      ...
      await fs.writeFile(path.join(testDir, 'index.js'), 'const express = require("express");');
  ...
  describe('Multi-Framework Detection', () => {
    it('should detect multiple frameworks in hybrid projects', async () => {
      ...
      await fs.writeFile(path.join(testDir, 'vite.config.ts'), 'import { defineConfig } from "vite"');
      await fs.writeFile(path.join(testDir, 'tsconfig.json'), '{"compilerOptions": {"jsx": "react-jsx"}}');
    ...
    it('should handle Next.js with App Router configuration', async () => {
      ...
      await fs.writeFile(path.join(testDir, 'app', 'layout.tsx'), 'export default function RootLayout() {}');
      await fs.writeFile(path.join(testDir, 'app', 'page.tsx'), 'export default function HomePage() {}');
