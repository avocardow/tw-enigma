...
describe('JsExtractor', () => {
  ...
  describe('File Operations', () => {
    ...
    it('should reject files that are too large', async () => {
      ...
      await expect(extractor.extractFromFile('large.js')).rejects.toThrow(JsFileReadError);
    });

    it('should handle file read errors', async () => {
      mockFs.stat.mockRejectedValue(new Error('File not found'));
      
      await expect(extractor.extractFromFile('missing.js')).rejects.toThrow(JsFileReadError);
    });
    ...
    it('should process multiple files', async () => {
      ...
      const results = await extractor.extractFromFiles(['file1.js', 'file2.js']);
