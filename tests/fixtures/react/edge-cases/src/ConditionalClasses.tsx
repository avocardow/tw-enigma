...
export function ConditionalClasses({
  ...
  const renderIcon = () => {
    ...
    return (
      <span className={`mr-2 ${variant === 'danger' ? 'text-red-200' : variant === 'success' ? 'text-green-200' : 'text-blue-200'}`}>
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className={`grid gap-4 ${fullWidth ? 'grid-cols-1' : 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4'}`}>
        {Array.from({ length: 8 }, (_, i) => (
          ...
          >
            <div className={`text-center ${i % 2 === 0 ? 'text-blue-700' : 'text-purple-700'}`}>
              ...
              <div className={`mt-2 h-2 rounded-full ${count > i ? 'bg-current' : 'bg-gray-200'} transition-colors duration-500`}></div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-4">
        {[
          { condition: count > 3, text: 'Great job!', classes: 'bg-green-100 text-green-800 border-green-200' },
          { condition: count > 7, text: 'You\'re on fire!', classes: 'bg-orange-100 text-orange-800 border-orange-200' },
          { condition: count > 12, text: 'Absolutely amazing!', classes: 'bg-purple-100 text-purple-800 border-purple-200' },
          { condition: count > 20, text: 'You\'re a legend!', classes: 'bg-yellow-100 text-yellow-800 border-yellow-200' }
        ].map((item, index) => (
          item.condition && (
            <div
              key={index}
...
export function withConditionalStyling<T extends Record<string, any>>(
  ...
  return function ConditionalStyledComponent(props: T & { 
    theme?: 'light' | 'dark'; 
