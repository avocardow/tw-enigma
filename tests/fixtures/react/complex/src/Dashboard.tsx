...
const Dashboard: React.FC<DashboardProps> = ({ users, theme }) => {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${
      ...
      {/* Header */}
      <header className={`sticky top-0 z-50 shadow-lg border-b backdrop-blur-sm ${
        theme === 'dark'
        ...
        {!isLoading && (
          <div className={selectedView === 'grid' 
            ...
            {filteredUsers.map((user) => (
              ...
              >
                {/* Avatar */}
                <div className={`${selectedView === 'grid' ? 'mb-4' : ''} flex-shrink-0`}>
                  <div className={`${
                    selectedView === 'grid' ? 'w-16 h-16 mx-auto' : 'w-12 h-12'
                ...
                {/* Content */}
                <div className={`${selectedView === 'grid' ? 'text-center' : 'flex-1'}`}>
                  <h3 className={`font-semibold ${selectedView === 'grid' ? 'text-lg mb-1' : 'text-base'}`}>
                    {user.name}
                  </h3>
                  <p className={`text-sm ${
                  ...
                  {/* Badges */}
                  <div className={`flex ${
                    selectedView === 'grid' ? 'justify-center' : 'justify-start'
                  } space-x-2 mb-2`}>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${roleClasses[user.role]}`}>
                      {user.role}
                    </span>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${statusClasses[user.status]}`}>
                      {user.status}
                    </span>
                  </div>
