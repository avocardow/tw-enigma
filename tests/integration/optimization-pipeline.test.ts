...
import { HtmlExtractor, JsExtractor } from '@tw-enigma/core';
import { FixtureLoader, FixtureProject } from '../utils/fixture-loader';
import { PerformanceUtils, performanceMonitor } from '../utils/performance-monitor';
...
describe('Optimization Pipeline Integration Tests', () => {
  ...
  describe('Complexity-based performance', () => {
    ...
    it('should handle edge cases gracefully', async () => {
      ...
      for (const fixture of edgeCaseFixtures) {
        ...
        expect(hasColorClasses || hasHoverStates || hasPseudoClasses || hasResponsiveClasses).toBe(
          true
        );
      }
    });
  ...
  describe('Memory and performance benchmarks', () => {
    it('should maintain stable memory usage across multiple runs', async () => {
      ...
      const variance =
        memoryMeasurements.reduce((sum, val) => sum + Math.pow(val - avgMemory, 2), 0) /
    ...
    it('should show reasonable performance scaling', async () => {
      ...
      for (const [complexity, fixtureList] of Object.entries(fixturesByComplexity)) {
        ...
        for (const fixture of fixtureList.slice(0, 3)) {
          ...
          const { duration } = await PerformanceUtils.time(() => runOptimizationPipeline(fixture));
      ...
      if (avgPerformance.simple && avgPerformance.complex) {
        ...
        const relativeVariance =
          timeDifference / Math.max(avgPerformance.simple, avgPerformance.complex);

        console.log(
          `Time difference: ${timeDifference.toFixed(6)}ms, relative variance: ${(relativeVariance * 100).toFixed(2)}%`
        );
        ...
        if (relativeVariance < 0.5) {
          console.warn(
            'Performance timings are close or inverted, skipping strict scaling assertion due to CI timing variance'
          );
        } else {
  ...
  function expectReactSpecificOptimizations(metrics: PipelineTestMetrics): void {
    ...
    const reactPatterns = classes.filter(
      (c) => c.includes('flex') || c.includes('grid') || c.includes('bg-') || c.includes('text-')
    );
  ...
  function printTestSummary(): void {
    ...
    const frameworkStats = ['react', 'nextjs', 'vite']
      .map((framework) => {
        ...
        const avgRatio =
          frameworkResults.reduce((sum, m) => sum + m.result.optimizationRatio, 0) /
          frameworkResults.length;
        const avgTime =
          frameworkResults.reduce((sum, m) => sum + m.performance.overall.duration, 0) /
          frameworkResults.length;
