...
import { PerformanceMonitor, PerformanceUtils, performanceMonitor } from '../utils/performance-monitor';
...
describe('Optimization Pipeline Integration Tests', () => {
  ...
  let performanceResults: Map<string, PipelineTestMetrics> = new Map();
  ...
  beforeAll(async () => {
    try {
      ...
      fixtures = await fixtureLoader.loadAllFixtures({
        ...
        includeExtensions: ['.tsx', '.ts', '.jsx', '.js', '.vue', '.html', '.css']
      });

      console.log(`\nLoaded ${fixtures.length} test fixtures:`);
      for (const fixture of fixtures) {
        const stats = fixtureLoader.getFixtureStats(fixture);
        console.log(`  - ${fixture.name}: ${stats.totalFiles} files, ${PerformanceUtils.formatBytes(stats.totalSize)}`);
      }
  ...
  afterAll(async () => {
    ...
    const resultsPath = join(process.cwd(), 'tests/results/integration-performance.json');
  ...
  describe('Framework-specific optimizations', () => {
    it('should optimize React fixtures correctly', async () => {
      const reactFixtures = (fixtures || []).filter(f => f.framework === 'react');
      ...
      for (const fixture of reactFixtures) {
        ...
        expect(metrics.performance.overall.memoryDelta).toBeLessThan(100 * 1024 * 1024); // 100MB max
    ...
    it('should optimize Next.js fixtures correctly', async () => {
      const nextjsFixtures = (fixtures || []).filter(f => f.framework === 'nextjs');
    ...
    it('should optimize Vite/Vue fixtures correctly', async () => {
      const viteFixtures = (fixtures || []).filter(f => f.framework === 'vite');
  ...
  describe('Complexity-based performance', () => {
    it('should handle simple fixtures efficiently', async () => {
      const simpleFixtures = (fixtures || []).filter(f => f.complexity === 'simple');
      const benchmarks = [];
      ...
      for (const fixture of simpleFixtures) {
        ...
        expect(result.summary.memoryPeak).toBeLessThan(50 * 1024 * 1024); // 50MB
      }

      // Verify consistency across runs (very relaxed variance expectation for integration tests)
      for (const benchmark of benchmarks) {
        const variance = (benchmark.summary.maxDuration - benchmark.summary.minDuration) / benchmark.summary.averageDuration;
    ...
    it('should scale reasonably with complex fixtures', async () => {
      const complexFixtures = (fixtures || []).filter(f => f.complexity === 'complex');
      ...
      for (const fixture of complexFixtures) {
        ...
        expect(metrics.performance.overall.memoryDelta).toBeLessThan(200 * 1024 * 1024); // 200MB
    ...
    it('should handle edge cases gracefully', async () => {
      const edgeCaseFixtures = (fixtures || []).filter(f => f.complexity === 'edge-cases');
      ...
      for (const fixture of edgeCaseFixtures) {
        ...
        expect(hasColorClasses || hasHoverStates || hasPseudoClasses || hasResponsiveClasses).toBe(true);
      }
    });
  });
  ...
  describe('Optimization quality metrics', () => {
    it('should achieve meaningful bundle size reductions', async () => {
      ...
      for (const metrics of results) {
        ...
        expect(metrics.result.outputSize).toBeLessThan(fullTailwindEstimate * 0.1); // <10% of full size
      }
    });
    ...
    it('should maintain CSS functionality', async () => {
      for (const fixture of fixtures) {
        ...
        const cssRules = metrics.result.generatedCss.match(/\.[^{]+\{[^}]+\}/g) || [];
    ...
    it('should handle responsive and state variants correctly', async () => {
      const complexFixtures = fixtures.filter(f => 
        f.complexity === 'complex' || f.complexity === 'edge-cases'
      );
  ...
  describe('Memory and performance benchmarks', () => {
    it('should maintain stable memory usage across multiple runs', async () => {
      const fixture = fixtures.find(f => f.framework === 'react' && f.complexity === 'simple');
      if (!fixture) return;
      ...
      const variance = memoryMeasurements.reduce((sum, val) => sum + Math.pow(val - avgMemory, 2), 0) / memoryMeasurements.length;
    ...
    it('should show reasonable performance scaling', async () => {
      ...
      for (const [complexity, fixtureList] of Object.entries(fixturesByComplexity)) {
        performanceByComplexity[complexity] = [];
        
        for (const fixture of fixtureList.slice(0, 3)) { // Test 3 fixtures per complexity
  ...
  // Helper functions
  async function runOptimizationPipeline(fixture: FixtureProject): Promise<PipelineTestMetrics> {
    const errors: string[] = [];
    ...
    try {
      ...
      const extractionResult = await PerformanceUtils.measureMemory(async () => {
        const timer = performanceMonitor.startOperation(`extract-${fixture.name}`);
        ...
        try {
          ...
          const componentFiles = fixtureLoader.filterFiles(fixture, { type: 'component' });
          
          for (const file of componentFiles) {
            let classes: string[] = [];
            
            if (['.html'].includes(file.extension)) {
              const result = await htmlExtractor.extractFromString(file.content, file.path);
              classes = Array.from(result.classes.keys());
            } else if (['.tsx', '.ts', '.jsx', '.js', '.vue'].includes(file.extension)) {
              const result = await jsExtractor.extractFromString(file.content, file.path);
              classes = Array.from(result.classes.keys());
            }
            ...
            if (classes.length > 0) {
              console.log(`File ${file.path}: Found ${classes.length} raw classes, ${validClasses.length} valid classes`);
          ...
          return { classes: Array.from(allClasses), duration: metrics.duration };
        } catch (error) {
          timer.end();
          throw error;
        }
      ...
      const generationResult = await PerformanceUtils.measureMemory(async () => {
        const timer = performanceMonitor.startOperation(`generate-${fixture.name}`);
        ...
        try {
          ...
          const css = classes.map(className => {
            ...
            } else if (className.startsWith('p-') || className.startsWith('px-') || className.startsWith('py-')) {
      ...
      const optimizationRatio = extractedClassCount > 0 && outputSize > 0
      ...
      const result: OptimizationResult = {
        ...
        optimizationRatio,
        processingTime: extractionResult.result.duration + generationResult.result.duration,
        memoryUsage: extractionResult.memoryDelta + generationResult.memoryDelta
      };
  ...
  function expectReactSpecificOptimizations(metrics: PipelineTestMetrics): void {
    const classes = metrics.result.extractedClasses;
    ...
    const reactPatterns = classes.filter(c => 
      c.includes('flex') || c.includes('grid') || c.includes('bg-') || c.includes('text-')
    );
    expect(reactPatterns.length).toBeGreaterThan(0);
  }

  function expectNextjsSpecificOptimizations(metrics: PipelineTestMetrics): void {
    const classes = metrics.result.extractedClasses;
  ...
  function expectViteSpecificOptimizations(metrics: PipelineTestMetrics): void {
    ...
    const vuePatterns = classes.filter(c => 
      c.includes('transition') || c.includes('duration') || c.includes('ease') ||
      c.includes('bg-') || c.includes('text-') || c.includes('p-') ||
      c.includes('rounded') || c.includes('shadow') || c.includes('border') ||
      c.includes('min-h') || c.includes('flex') || c.includes('sticky')
    );
  ...
  function printTestSummary(): void {
    ...
    const avgOptimization = Array.from(performanceResults.values())
      .reduce((sum, metrics) => sum + metrics.result.optimizationRatio, 0) / performanceResults.size;
    
    const avgProcessingTime = Array.from(performanceResults.values())
      .reduce((sum, metrics) => sum + metrics.performance.overall.duration, 0) / performanceResults.size;
    
    console.log(`Average optimization ratio: ${(avgOptimization * 100).toFixed(1)}%`);
    console.log(`Average processing time: ${PerformanceUtils.formatDuration(avgProcessingTime)}`);
    
    // Framework breakdown
    const frameworkStats = ['react', 'nextjs', 'vite'].map(framework => {
      const frameworkResults = Array.from(performanceResults.values())
        .filter(m => m.fixture.framework === framework);
      
      if (frameworkResults.length === 0) return null;
      
      const avgRatio = frameworkResults.reduce((sum, m) => sum + m.result.optimizationRatio, 0) / frameworkResults.length;
      const avgTime = frameworkResults.reduce((sum, m) => sum + m.performance.overall.duration, 0) / frameworkResults.length;
    ...
    for (const stats of frameworkStats) {
      console.log(`  ${stats!.framework}: ${stats!.count} tests, ${(stats!.avgOptimization * 100).toFixed(1)}% optimization, ${PerformanceUtils.formatDuration(stats!.avgTime)} avg`);
    }
  }
}); 