...
class IncrementalAnalyzer {
  ...
  async analyzeIncremental(changedFiles: string[], allFiles: string[]): Promise<AnalysisResult> {
    ...
    const partialResults = await Promise.all(affectedFiles.map((file) => this.analyzeFile(file)));
...
class SecureConfig {
  ...
  private sanitizeConfig(config: Configuration): Configuration {
    ...
    if (sanitized.apiKeys) {
      sanitized.apiKeys = Object.keys(sanitized.apiKeys).reduce(
        (acc, key) => {
          acc[key] = '***masked***';
          return acc;
        },
