...
export class PerformanceValidator extends EventEmitter {
  ...
  public async analyzePerformance(
    ...
    try {
      ...
      const ioAnalysis = analyzeFileSystem
        ? await this.analyzeIoImpact()
        : { score: 100, warnings: [], bottlenecks: [] };
      const networkAnalysis = checkNetworkImpact
        ? await this.analyzeNetworkImpact()
        : { score: 100, warnings: [], bottlenecks: [] };
      ...
      if (
        this.config.runtime &&
        typeof this.config.runtime.checkInterval === 'number' &&
        this.config.runtime.checkInterval < 1000
      ) {
        warnings.push({
          type: 'timeout',
          severity: 'warning',
  ...
  private async analyzeMemoryImpact(): Promise<{
    ...
    if (estimatedMemoryPerWorker < 64 * 1024 * 1024) {
      // Less than 64MB per worker
      bottlenecks.push({
  ...
  private async generateRecommendations(..._analyses: any[]): Promise<PerformanceRecommendation[]> {
    ...
    if (!this.config.output) {
      // Use output as a proxy for cache configuration
      recommendations.push({
        type: 'configuration',
        priority: 'high',
 ...
 */
export { PerformanceValidator as PerformanceValidatorClass };
