...
export class DevDiagnostics extends EventEmitter {
  ...
  private startFileWatching(): void {
    const patterns = ['src/**/*.{css,html,js,jsx,ts,tsx}', 'styles/**/*.css', 'public/**/*.html'];
  ...
  private checkThresholds(metrics: DevPerformanceMetrics): void {
    ...
    if (memoryMB > this.config.thresholds.memoryError) {
      ...
      this.logger.error('Memory usage critical', {
        memoryMB,
        threshold: this.config.thresholds.memoryError,
      });
    ...
    if (cpuPercent > this.config.thresholds.cpuError) {
      ...
      this.logger.error('CPU usage critical', {
        cpuPercent,
        threshold: this.config.thresholds.cpuError,
      });
...
interface DevDiagnosticsEventEmitter {
  ...
  emit<K extends keyof DiagnosticsEvents>(
    event: K,
