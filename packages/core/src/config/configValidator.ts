...
export class ConfigValidator {
  ...
  private validationRules: Map<
    string,
    (value: unknown, _context: ValidationContext) => ValidationResult
  >;
  ...
  private initializeValidationRules(): void {
    ...
    this.validationRules.set(
      ...
      this.validateOptimizationSettings.bind(this)
    );
  }
  ...
  public async validateFile(
    filepath: string
  ): Promise<ValidationResult & { config?: EnigmaConfig }> {
    try {
   ...
   */
  private async validateBasicSchema(
    config: unknown,
    filepath?: string
  ): Promise<ValidationResult & { config?: EnigmaConfig }> {
    try {
  ...
  private async validateCrossFields(
    ...
    if (config.input && config.output) {
      ...
      const outputPath =
        typeof config.output === 'string' ? resolve(config.output) : resolve('./dist');
          ...
          new ValidationError(
            ...
            undefined,
            {
              ...
              suggestions: [
                ...
                'Add a subdirectory to the output path',
              ],
            }
          )
        );
    ...
    if (config.maxConcurrency && config.htmlExtractor?.maxFileSize) {
      const memoryEstimate =
        config.maxConcurrency * (config.htmlExtractor.maxFileSize / (1024 * 1024));
      if (memoryEstimate > 1024) {
        // > 1GB estimated memory usage
        warnings.push(
          `High memory usage estimated: ${memoryEstimate.toFixed(0)}MB with current concurrency and file size settings`
        );
        suggestions.push(
          'Consider reducing maxConcurrency or maxFileSize to optimize memory usage'
        );
      }
      rulesApplied++;
    }
    ...
    if (config.dev?.enabled) {
      if (config.minify && config.dev.diagnostics?.enabled) {
        ...
        suggestions.push(
          'Consider disabling minification in development mode for better debugging'
        );
      }
      rulesApplied++;
    }
  ...
  private async validateSecurity(
    ...
    const pathFields = [
      config.input,
      typeof config.output === 'string' ? config.output : undefined,
    ];
    for (const path of pathFields) {
      if (path && typeof path === 'string' && (path.includes('../') || path.includes('..\\'))) {
        errors.push(
          new ValidationError('Path traversal detected in configuration', 'path', path, undefined, {
    ...
    if (config.output && typeof config.output === 'string') {
      try {
        ...
        warnings.push(
          `Cannot verify output directory permissions: ${error instanceof Error ? error.message : String(error)}`
        );
      }
      rulesApplied++;
    }
  ...
  private validateConcurrencyLimits(
    _value: unknown,
  ...
  private validateOptimizationSettings(
    _value: unknown,
    _context: ValidationContext
  ): ValidationResult {
...
export async function validateConfigSchema(
  config: unknown
): Promise<{ success: boolean; data?: EnigmaConfig; errors?: string[] }> {
