...
describe('JsExtractor', () => {
  ...
  describe('Zod Schema Validation', () => {
    ...
    it('should reject invalid options', () => {
      ...
      expect(() => JsExtractionOptionsSchema.parse({ maxFileSize: 0 })).toThrow();
    });
  });
  ...
  describe('Static Class Extraction', () => {
    ...
    it('should track line numbers and patterns', async () => {
      ...
      expect(containerClass?.contexts[0].pattern).toContain('className="container"');
    });
  });
  ...
  describe('Statistics and Metadata', () => {
    it('should provide comprehensive statistics', async () => {
      ...
      expect(result.metadata.extractionStats.templateMatches).toBeGreaterThan(0);
  ...
  describe('Error Handling', () => {
    it('should handle parsing errors gracefully', async () => {
      ...
      await expect(extractor.extractFromString(code)).rejects.toThrow(JsParsingError);
    });
  ...
  describe('File Operations', () => {
    ...
    it('should extract classes from file', async () => {
      ...
      expect(result.classes.has('container')).toBe(true);
      expect(result.metadata.source).toBe('test.ts');
      expect(result.metadata.fileSize).toBe(1000);
      expect(mockFs.stat).toHaveBeenCalledWith('test.ts');
    ...
    it('should reject files that are too large', async () => {
      ...
      await expect(extractor.extractFromFile('large.ts')).rejects.toThrow(JsFileReadError);
    });

    it('should handle file read errors', async () => {
      mockFs.stat.mockRejectedValue(new Error('File not found'));

      await expect(extractor.extractFromFile('missing.ts')).rejects.toThrow(JsFileReadError);
    });
    ...
    it('should process multiple files', async () => {
      ...
      const results = await extractor.extractFromFiles(['file1.ts', 'file2.ts']);
  ...
  describe('Timeout Handling', () => {
    it('should handle file read timeout', async () => {
      ...
      await expect(extractor.extractFromFile('slow.ts')).rejects.toThrow(
...
describe('Factory Functions', () => {
  ...
  it('should extract classes from file with convenience function', async () => {
    ...
    const result = await extractClassesFromJsFile('test.ts');
