...
import { Logger, LogLevel, LogLevelNames, createLogger, ErrorContext } from '@tw-enigma/core';
...
describe('Logger', () => {
  ...
  describe('Performance Timing', () => {
    ...
    it('should include additional context in timing logs', () => {
      ...
      expect(consoleOutput).toHaveLength(1);
      const logEntry = JSON.parse(consoleOutput[0]);
      expect(logEntry.context.filePath).toBe('/test/file.ts');
  ...
  describe('Default Logger Instance', () => {
    it('should have sensible defaults for development', async () => {
      ...
      vi.resetModules();
      const { logger: devLogger } = await import('../src/utils/logger');
    ...
    it('should respect environment variables', async () => {
      ...
      vi.resetModules();
      const { logger: verboseLogger } = await import('../src/utils/logger');
...
describe('Enhanced Logger Features', () => {
  ...
  describe('Progress Tracking', () => {
    ...
    it('should complete progress tracking', () => {
      ...
      testLogger.completeProgress('complete-test', 'Successfully processed all items');

      expect(consoleOutput).toHaveLength(2);
      expect(consoleOutput[1]).toContain('✅ Completed Complete Test (10 items');
  ...
  describe('Performance Metrics', () => {
    it('should log performance metrics in very verbose mode', () => {
      ...
      const logEntry = JSON.parse(consoleOutput[0]);
      expect(logEntry.level).toBe('TRACE');
      expect(logEntry.message).toContain('CSS Optimization completed in 1500ms');
  ...
  describe('Process Step Logging', () => {
    it('should log process steps in verbose mode', () => {
      ...
      const logEntry = JSON.parse(consoleOutput[0]);
      expect(logEntry.level).toBe('DEBUG');
      expect(logEntry.message).toBe('🔄 Analyzing CSS files: Found 10 stylesheets');
    });
