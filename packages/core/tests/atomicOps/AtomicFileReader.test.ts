...
describe('AtomicFileReader', () => {
  ...
  describe('Batch Operations', () => {
    ...
    it('should read multiple files successfully', async () => {
      const results = await reader.readMultipleFiles(batchFiles.map((f) => ({ path: f.path })));
    ...
    it('should handle mixed success/failure in batch', async () => {
      const files = [{ path: TEST_FILE }, { path: MISSING_FILE }, { path: TEST_JSON_FILE }];
  ...
  describe('Resource Management', () => {
    ...
    it('should handle concurrent reads safely', async () => {
      const promises = Array.from({ length: 10 }, () => reader.readFile(TEST_FILE));
