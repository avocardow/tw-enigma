...
describe('CssInjector', () => {
  ...
  describe('Constructor and Configuration', () => {
    ...
    test('should validate required options', () => {
      ...
      expect(() => new CssInjector({ htmlPath: '' })).toThrow(CssInjectionError);
    });
      ...
      expect(
        () =>
          new CssInjector({
            cssPath: 'test.css',
            htmlPath: 'test.html',
            insertPosition: 'invalid' as 'first' | 'last' | 'before-existing' | 'after-meta',
          })
  ...
  describe('HTML Document Structure Analysis', () => {
    ...
    test('should detect existing stylesheets', async () => {
      ...
      expect(result.documentStructure.existingLinks[0].href).toBe('existing.css');
      expect(result.documentStructure.existingLinks[1].href).toBe('another.css');
    });
    ...
    test('should detect existing style tags', async () => {
      ...
      expect(result.documentStructure.existingStyles[0].content).toBe('body { margin: 0; }');
...
\t</body>
    ...
    test('should throw error for documents without head when createHeadIfMissing is false', async () => {
      ...
      await expect(testInjector.injectIntoString(html)).rejects.toThrow(HtmlStructureError);
    });
  });
...
<html>
  ...
  describe('Error Handling', () => {
    ...
    test('should respect file size limits', async () => {
      ...
      const html = '<html><head></head><body>This is longer than 10 bytes</body></html>';

      await expect(testInjector.injectIntoString(html)).rejects.toThrow(CssInjectionError);
    });
...
describe('Custom Error Classes', () => {
  test('CssInjectionError should include source and code', () => {
    const error = new CssInjectionError('Test message', 'test-source', undefined, 'TEST_CODE');
  ...
  test('PathCalculationError should include path information', () => {
    const error = new PathCalculationError('Path calculation failed', '/from/path', '/to/path');
  ...
  test('HtmlStructureError should include HTML content', () => {
    const error = new HtmlStructureError('Invalid structure', '<invalid>html</invalid>');
