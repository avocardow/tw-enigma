...
describe('FileIntegrityValidator', () => {
  ...
  describe('Checksum Calculation', () => {
    ...
    it('should throw error for non-existent file', async () => {
      ...
      await expect(validator.calculateChecksum(nonExistentFile)).rejects.toThrow(ChecksumError);
    });
    ...
    it('should throw error for file exceeding size limit', async () => {
      ...
      await expect(smallSizeValidator.calculateChecksum(testFile)).rejects.toThrow(ChecksumError);
    });
  ...
  describe('File Comparison', () => {
    ...
    it('should handle comparison errors', async () => {
      ...
      await expect(validator.compareFiles(testFile, nonExistentFile)).rejects.toThrow(
        ValidationError
      );
    });
  });
  ...
  describe('Backup Operations', () => {
    ...
    it('should handle backup of non-existent file', async () => {
      ...
      await expect(validator.createBackup(nonExistentFile)).rejects.toThrow(RollbackError);
    });
  });
  ...
  describe('Rollback Operations', () => {
    ...
    it('should handle rollback of non-existent backup', async () => {
      ...
      await expect(validator.restoreFromBackup(testFile, nonExistentBackup)).rejects.toThrow(
        RollbackError
      );
    });
    ...
    it('should handle rollback verification failure', async () => {
      ...
      const compareFilesSpy = vi
        .spyOn(verifyingValidator as any, 'compareFiles')
        .mockResolvedValue({
          ...
          checksum2: {
            ...
            algorithm: 'sha256',
            fileSize: 100,
            filePath: verifyBackupResult.backupPath,
            timestamp: new Date(),
            processingTime: 10,
          },
          processingTime: 20,
      ...
      await expect(
        verifyingValidator.restoreFromBackup(testFile, verifyBackupResult.backupPath)
  ...
  describe('Large Project Optimization', () => {
    ...
    describe('Batch Processing Framework', () => {
      ...
      it('should handle batch processing errors gracefully', async () => {
        ...
        const result = await largeProjectValidator.processLargeProject(filesWithError, 'checksum', {
      ...
      it('should support different batch processing strategies', async () => {
        ...
        for (const strategy of strategies) {
          ...
          expect(
            result.optimizationDetails.some((detail) => detail.includes('Batch processing enabled'))
          ).toBe(true);
        }
      });
    });
    ...
    describe('Performance Optimization', () => {
      ...
      it('should provide optimization recommendations', async () => {
        ...
        expect(
          result.optimizationDetails.some((detail) => detail.includes('Memory tracking'))
        ).toBe(true);
        expect(
          result.optimizationDetails.some((detail) => detail.includes('System metrics monitoring'))
        ).toBe(true);
      });
    });
    ...
    describe('Large File Set Handling', () => {
      ...
      it('should handle mixed success/failure scenarios gracefully', async () => {
        ...
        let errorCount = 0;
        const result = await largeProjectValidator.processLargeProject(mixedFiles, 'checksum', {
