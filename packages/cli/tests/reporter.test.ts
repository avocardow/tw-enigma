...
describe('Reporter', () => {
  ...
  describe('Recommendations and Warnings', () => {
    ...
    it('should generate size-based warnings', () => {
      ...
      expect(warnings).toContain('Optimization resulted in larger file size - check configuration');
    });
    ...
    it('should generate performance warnings', () => {
      ...
      expect(warnings).toContain('Optimization is taking very long - consider timeout settings');
      expect(warnings).toContain('Very high memory usage - risk of out-of-memory errors');
    });
