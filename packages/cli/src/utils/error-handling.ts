...
export function handleCLIError(error: unknown, logger: Logger): never {
  if (error instanceof CLIError) {
    ...
    if (error.details && logger.level <= 2) { // DEBUG level or lower
...
export const CLIErrors = {
  configNotFound: (path: string) => new CLIError(
  ...
  configInvalid: (errors: string[]) => new CLIError(
  ...
  fileNotFound: (path: string) => new CLIError(
    `File not found: ${path}`,
    CLIErrorType.FILE_ERROR,
    1,
    { filePath: path }
  ),

  fileNotAccessible: (path: string, reason: string) => new CLIError(
    `Cannot access file: ${path}`,
    CLIErrorType.FILE_ERROR,
    1,
    { filePath: path, reason }
  ),

  invalidInput: (message: string, input?: unknown) => new CLIError(
  ...
  processingFailed: (operation: string, reason: string) => new CLIError(
  ...
  networkError: (operation: string, error: string) => new CLIError(
    `Network error during ${operation}: ${error}`,
    CLIErrorType.NETWORK_ERROR,
    1,
    { operation, networkError: error }
  )
};
...
export function validateRequiredOptions(
  ...
  if (missing.length > 0) {
    throw CLIErrors.invalidInput(
      `Missing required options: ${missing.join(', ')}`,
      { missing, provided: Object.keys(options) }
    );
  }
} 