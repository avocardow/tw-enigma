...
import { CircuitBreakerRegistry, CircuitBreaker } from './circuitBreaker.js';
import {
  ErrorSeverity,
  ErrorCategory,
...
export class ErrorHandler extends EventEmitter {
   ...
   */
  private async checkAlertThresholds(severity: ErrorSeverity, category: ErrorCategory): Promise<void> {
  ...
  getAnalytics(): ErrorAnalytics {
    ...
    return {
      ...
      recoveryRate: stats.recoveryAttempts > 0 
  ...
  private calculateSystemHealth(
    stats: ErrorStats, 
    circuitHealth: { healthy: number; degraded: number; unhealthy: number; total: number }
  ): HealthStatus {
