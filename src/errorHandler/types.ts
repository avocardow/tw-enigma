...
export interface ShutdownContext {
  ...
  cleanupTasks: Array<{           // Cleanup tasks to execute
    name: string;
    priority: number;
    timeout: number;
  }>;
...
export interface HealthCheckResult {
  ...
  checks: Record<string, {        // Individual check results
...
export function isOperationalError(error: Error): boolean {
  return error.name.includes('File') || 
...
export function isProgrammingError(error: Error): boolean {
  return error.name.includes('Type') || 
...
export function categorizeError(error: Error): ErrorCategory {
  ...
  if (name.includes('config') || message.includes('configuration') || 
  ...
  if (name.includes('validation') || message.includes('invalid') || 
      message.includes('schema') || name.includes('zod')) {
