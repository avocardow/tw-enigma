 ...
 */
export const webpackPluginConfigSchema = z.object({
  name: z.string(),
...
export class EnigmaWebpackPlugin implements WebpackPluginInstance, BuildToolPlugin {
  ...
  private registerHooks(compiler: Compiler): void {
    // Before build starts
    compiler.hooks.beforeRun.tapAsync('EnigmaWebpackPlugin', async (compiler, callback) => {
      try {
        if (this.context) {
    ...
    // Emit hook
    compiler.hooks.emit.tapAsync('EnigmaWebpackPlugin', async (compilation, callback) => {
      try {
        if (this.context) {
  ...
  private setupHMR(compiler: Compiler): void {
    ...
    try {
      ...
      compiler.hooks.watchRun.tapAsync('EnigmaWebpackPlugin', async (compiler, callback) => {
        try {
          if (this.context) {
  ...
  private async processAssets(compilation: any, assets: any): Promise<void> {
    const cssAssets = Object.keys(assets).filter(name => 
    ...
    for (const assetName of cssAssets) {
      try {
        ...
        if (optimizedResult) {
          ...
          compilation.updateAsset(assetName, new compilation.constructor.webpack.sources.RawSource(optimizedResult.css));
  ...
  getBuildToolConfig(): Configuration {
    ...
    const hasEnigmaPlugin = config.plugins.some(plugin => 
