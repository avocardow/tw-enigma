...
import type { CssPerformanceReport, BudgetViolation } from './cssReportGenerator.js';
...
export class CiIntegration {
  ...
  async processReport(report: CssPerformanceReport): Promise<{
    ...
    if (this.options.minPerformanceScore && report.metrics.performanceScore < this.options.minPerformanceScore) {
      success = false;
      exitCode = 1;
      messages.push(`❌ Performance score ${report.metrics.performanceScore} below threshold ${this.options.minPerformanceScore}`);
    } else {
    ...
    if (this.options.failOnBudgetViolation && !report.budgetAnalysis.passed) {
      success = false;
      exitCode = 1;
      const errorViolations = report.budgetAnalysis.violations.filter(v => v.severity === 'error');
    ...
    if (comparison && this.options.maxSizeIncrease) {
      ...
      if (sizeIncreasePercent > this.options.maxSizeIncrease) {
        success = false;
        exitCode = 1;
        messages.push(`❌ Size increase ${sizeIncreasePercent.toFixed(1)}% exceeds threshold ${this.options.maxSizeIncrease}%`);
      }
    }
   ...
   */
  private compareReports(current: CssPerformanceReport, baseline: CssPerformanceReport): PerformanceComparison {
    const delta = {
      scoreChange: current.metrics.performanceScore - baseline.metrics.performanceScore,
      sizeChange: ((current.metrics.totalCompressedSize - baseline.metrics.totalCompressedSize) / baseline.metrics.totalCompressedSize) * 100,
      loadTimeChange: current.metrics.averageLoadTime - baseline.metrics.averageLoadTime,
      compressionChange: current.metrics.overallCompressionRatio - baseline.metrics.overallCompressionRatio
    };
  ...
  private detectRegressions(
    ...
    if (delta.scoreChange < -5) {
      regressions.push({
        type: 'score_decrease',
        severity: delta.scoreChange < -15 ? 'major' : delta.scoreChange < -10 ? 'moderate' : 'minor',
    ...
    if (delta.loadTimeChange > 500) {
      regressions.push({
        ...
        severity: delta.loadTimeChange > 2000 ? 'major' : delta.loadTimeChange > 1000 ? 'moderate' : 'minor',
    ...
    const newViolations = current.budgetAnalysis.violations.filter(cv => 
  ...
  private async saveBaseline(report: CssPerformanceReport): Promise<void> {
    ...
    try {
      ...
      await fs.mkdir(path.dirname(this.options.baselinePath), { recursive: true });
  ...
  private async generateCiOutputs(
    ...
    try {
      ...
      const reportPath = path.join(outputDir, `css-performance-${this.environment.buildId || 'latest'}.json`);
      ...
      const summaryPath = path.join(outputDir, `css-performance-${this.environment.buildId || 'latest'}.md`);
      await fs.writeFile(summaryPath, markdown);
      
      // Generate CI-specific summary
      if (comparison) {
        const ciSummary = this.generateCiSummary(report, comparison);
        const ciSummaryPath = path.join(outputDir, `ci-summary-${this.environment.buildId || 'latest'}.md`);
   ...
   */
  private generateCiSummary(report: CssPerformanceReport, comparison: PerformanceComparison): string {
    const lines = ['# CSS Performance Summary\n'];
    
    // Key metrics
    lines.push('## Key Metrics');
    lines.push(`- **Performance Score:** ${report.metrics.performanceScore}/100 ${comparison.delta.scoreChange >= 0 ? '📈' : '📉'} (${comparison.delta.scoreChange > 0 ? '+' : ''}${comparison.delta.scoreChange.toFixed(1)})`);
    lines.push(`- **Total Size:** ${Math.round(report.metrics.totalCompressedSize / 1024)}KB ${comparison.delta.sizeChange <= 0 ? '📉' : '📈'} (${comparison.delta.sizeChange > 0 ? '+' : ''}${comparison.delta.sizeChange.toFixed(1)}%)`);
    lines.push(`- **Load Time:** ${Math.round(report.metrics.averageLoadTime)}ms ${comparison.delta.loadTimeChange <= 0 ? '📉' : '📈'} (${comparison.delta.loadTimeChange > 0 ? '+' : ''}${comparison.delta.loadTimeChange.toFixed(0)}ms)`);
    ...
    if (comparison.regressions.length > 0) {
      ...
      comparison.regressions.forEach(regression => {
        lines.push(`- **${regression.type.replace('_', ' ')}:** ${regression.description} (${regression.severity})`);
      });
      lines.push('');
    }
