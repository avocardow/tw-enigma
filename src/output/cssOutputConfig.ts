...
export const ChunkingConfigSchema = z.object({
  ...
  maxSize: z.number().min(1024).default(50 * 1024), // 50KB default
  
  /** Minimum size per chunk in bytes */
  minSize: z.number().min(512).default(2 * 1024), // 2KB default
...
export const CriticalCssConfigSchema = z.object({
  ...
  maxSize: z.number().min(1024).default(14 * 1024), // 14KB (HTTP/2 initial window)
...
export class CssOutputConfigManager {
  ...
  async loadFromFile(searchFrom?: string): Promise<CssOutputConfig> {
    try {
      ...
      throw new Error(`Failed to load CSS output configuration: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  async loadFromSpecificFile(filePath: string): Promise<CssOutputConfig> {
    try {
      ...
      throw new Error(`Failed to load CSS output configuration from ${filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
   ...
   */
  applyPreset(preset: 'production' | 'development', overrides?: Partial<CssOutputConfig>): CssOutputConfig {
  ...
  async loadConfig(searchFrom?: string): Promise<{ config: CssOutputConfig; filepath?: string }> {
    const config = await this.loadFromFile(searchFrom);
    return { config };
  }
   ...
   */
  async loadConfigFromPath(filePath: string): Promise<{ config: CssOutputConfig; filepath: string }> {
    const config = await this.loadFromSpecificFile(filePath);
  ...
  validateConfiguration(): { valid: boolean; errors: string[] } {
    ...
    try {
      ...
      if (this.config.critical.enabled && this.config.critical.maxSize > this.config.chunking.maxSize) {
...
export const PerformanceBudgetSchema = z.object({
  ...
  maxTotalSize: z.number().min(1024).default(500 * 1024), // 500KB
  
  /** Maximum individual chunk size in bytes */
  maxChunkSize: z.number().min(1024).default(100 * 1024), // 100KB
  
  /** Maximum critical CSS size in bytes */
  maxCriticalSize: z.number().min(1024).default(14 * 1024), // 14KB
...
export class ProductionCssConfigManager extends CssOutputConfigManager {
  ...
  constructor(initialConfig?: Partial<CssOutputConfig>, budget?: PerformanceBudget) {
    super(initialConfig);
  ...
  applyCliOverrides(args: CliArgs): CssOutputConfig {
    ...
    if (args.minify !== undefined) {
      overrides.optimization = { ...this.config.optimization, minify: args.minify };
    }
  ...
  validateAgainstBudgets(results?: {
    ...
    if (results) {
      ...
      if (results.totalSize > budget.maxTotalSize) {
        errors.push(`Total CSS size (${Math.round(results.totalSize / 1024)}KB) exceeds budget (${Math.round(budget.maxTotalSize / 1024)}KB)`);
      }
      
      if (results.criticalSize > budget.maxCriticalSize) {
        errors.push(`Critical CSS size (${Math.round(results.criticalSize / 1024)}KB) exceeds budget (${Math.round(budget.maxCriticalSize / 1024)}KB)`);
      }
      
      if (results.chunkSizes.length > budget.maxChunks) {
        errors.push(`Number of chunks (${results.chunkSizes.length}) exceeds budget (${budget.maxChunks})`);
      }
      
      const oversizedChunks = results.chunkSizes.filter(size => size > budget.maxChunkSize);
      if (oversizedChunks.length > 0) {
        errors.push(`${oversizedChunks.length} chunks exceed size budget (${Math.round(budget.maxChunkSize / 1024)}KB)`);
      }
      
      if (results.compressionRatio < budget.minCompressionRatio) {
        errors.push(`Compression ratio (${Math.round(results.compressionRatio * 100)}%) below budget (${Math.round(budget.minCompressionRatio * 100)}%)`);
      }
      
      if (results.loadTime > budget.maxLoadTime) {
        errors.push(`Estimated load time (${results.loadTime}ms) exceeds budget (${budget.maxLoadTime}ms)`);
      }
      
      // Check warning thresholds
      const warningThreshold = budget.warningThresholds;
      
      if (results.totalSize > budget.maxTotalSize * warningThreshold.totalSize) {
        warnings.push(`Total CSS size approaching budget limit (${Math.round((results.totalSize / budget.maxTotalSize) * 100)}%)`);
      }
      
      if (results.criticalSize > budget.maxCriticalSize * warningThreshold.criticalSize) {
        warnings.push(`Critical CSS size approaching budget limit (${Math.round((results.criticalSize / budget.maxCriticalSize) * 100)}%)`);
      }
      
      if (results.chunkSizes.length > budget.maxChunks * warningThreshold.chunkCount) {
        warnings.push(`Number of chunks approaching budget limit (${Math.round((results.chunkSizes.length / budget.maxChunks) * 100)}%)`);
      }
      
      if (results.loadTime > budget.maxLoadTime * warningThreshold.loadTime) {
        warnings.push(`Load time approaching budget limit (${Math.round((results.loadTime / budget.maxLoadTime) * 100)}%)`);
      }
    }
  ...
  fromCliArgs(args: CliArgs): CssOutputConfig {
    ...
    if (args.minify !== undefined || args.compress !== undefined || args.sourceMaps !== undefined) {
      overrides.optimization = {
    ...
    if (args['performance-budget'] || args['max-critical-css'] || args['max-chunk-size']) {
      ...
      if (args['performance-budget']) {
        ...
        if (match) {
          ...
          budget.maxBundleSize = unit === 'KB' ? size * 1024 : unit === 'MB' ? size * 1024 * 1024 : size;
        }
      }
      ...
      if (args['max-critical-css']) {
        ...
        if (match) {
          const size = parseInt(match[1]);
          const unit = (match[2] || 'B').toUpperCase();
          budget.maxCriticalCssSize = unit === 'KB' ? size * 1024 : unit === 'MB' ? size * 1024 * 1024 : size;
        }
      }
      ...
      if (args['max-chunk-size']) {
        ...
        if (match) {
          const size = parseInt(match[1]);
          const unit = (match[2] || 'B').toUpperCase();
          budget.maxChunkSize = unit === 'KB' ? size * 1024 : unit === 'MB' ? size * 1024 * 1024 : size;
        }
      }
      
      if (Object.keys(budget).length > 0) {
        this.performanceBudget = { ...this.performanceBudget, ...budget } as PerformanceBudget;
      }
    }
    ...
    if (args.environment) {
      ...
      if (args.environment === 'development') {
        ...
        envConfig.optimization.minify = true;
        envConfig.compression.type = envConfig.compression.type === 'none' ? 'gzip' : envConfig.compression.type;
      }
   ...
   */
  createOptimizedPreset(target: 'cdn' | 'serverless' | 'spa' | 'ssr'): CssOutputConfig {
  ...
  calculatePerformanceBudget(args: Record<string, string>): PerformanceBudget {
    ...
    return {
      maxBundleSize: args['performance-budget'] ? parseSize(args['performance-budget']) : 100 * 1024,
      maxCriticalCssSize: args['max-critical-css'] ? parseSize(args['max-critical-css']) : 14 * 1024,
      maxChunkSize: args['max-chunk-size'] ? parseSize(args['max-chunk-size']) : 50 * 1024,
      maxTotalSize: args['max-total-size'] ? parseSize(args['max-total-size']) : 500 * 1024,
   ...
   */
  detectCIEnvironment(): { isCI: boolean; provider?: string; environment: string } {
    const isCI = !!(
      process.env.CI ||
    ...
    const environment = process.env.NODE_ENV || (isCI ? 'production' : 'development');
  ...
  createCIConfiguration(options: {
    ...
    if (options.performanceBudget || this.performanceBudget) {
      (config as any).performanceBudget = options.performanceBudget || this.performanceBudget;
    }
 ...
 */
export function parseCliArgs(args: string[] | Record<string, unknown>): CliArgs {
  ...
  if (typeof withDefaults.compress === 'string') {
    if (withDefaults.compress === 'true' || withDefaults.compress === 'gzip' || withDefaults.compress === 'brotli') {
      withDefaults.compress = true;
    } else if (withDefaults.compress === 'false' || withDefaults.compress === 'none') {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      console.warn('CLI argument parsing failed, using defaults:', error.errors);
      return CliArgsSchema.parse(withDefaults);
    }
    throw error;
  }
...
export function validatePerformanceBudget(budget: PerformanceBudget): {
  ...
  if (budget.maxBundleSize > 2 * 1024 * 1024) { // 2MB - more aggressive limit 
    errors.push('Bundle size limit is excessively large (>2MB)');
  } else if (budget.maxBundleSize > 1024 * 1024) { // 1MB
    warnings.push('Bundle size limit is very large (>1MB)');
  }
  
  if (budget.maxCriticalCssSize > 50 * 1024) { // 50KB - more aggressive limit
    errors.push('Critical CSS size limit is excessively large (>50KB)');
  } else if (budget.maxCriticalCssSize > 25 * 1024) { // 25KB
    warnings.push('Critical CSS size limit is very large (>25KB)');
  }
  
  if (budget.estimatedLoadTime > 15000) { // 15 seconds - more aggressive limit
    errors.push('Estimated load time is excessively slow (>15s)');
  } else if (budget.estimatedLoadTime > 10000) { // 10 seconds
    warnings.push('Estimated load time is very slow (>10s)');
  }
  
  if (budget.maxChunks > 50) { // More aggressive limit  
  ...
  switch (preset) {
    ...
    case 'serverless':
      return {
        ...
        compression: {
          ...baseConfig.compression,
          type: baseConfig.compression.type === 'none' ? 'gzip' : baseConfig.compression.type
        },
        optimization: {
          ...baseConfig.optimization,
          minify: true
 ...
 */
export function createPerformanceBudget(overrides?: Partial<PerformanceBudget>): PerformanceBudget {
...
export function validateProductionConfig(config: CssOutputConfig): {
  ...
  if (!config.compression) {
    ...
    suggestions.push('Add compression: { type: "auto" } for better performance');
  }

  // Check for logical inconsistencies
  if (config.strategy === 'single' && config.chunking) {
    warnings.push('Chunking configuration ignored when using single output strategy');
    suggestions.push('Use strategy: "chunked" if you want to use chunking configuration');
  }
  
  // Production-specific validations
  if (config.optimization?.sourceMap && config.strategy !== 'development') {
    warnings.push('Source maps enabled in production build - consider disabling for better performance');
  }
  ...
  if (config.compression?.type === 'none') {
    warnings.push('Compression disabled - consider enabling for better performance');
  }
  ...
  if (config.criticalCss?.enabled && config.criticalCss.maxSize > 14 * 1024) {
    warnings.push('Critical CSS size exceeds HTTP/2 initial window size (14KB)');
  }
  
  if (config.chunking?.strategy === 'size' && config.strategy === 'modular') {
    recommendations.push('Consider using component-based chunking for modular output strategy');
  }
  
  if (!config.hashing?.includeContent) {
    recommendations.push('Enable content-based hashing for better cache invalidation');
  }
  
  if (config.delivery?.preload === false && config.criticalCss?.strategy === 'preload') {
    errors.push('Critical CSS preload strategy requires delivery.preload to be enabled');
  }

  // Check for performance budget if present
  if ((config as any).performanceBudget) {
    const budgetValidation = validatePerformanceBudget((config as any).performanceBudget);
...
export function generateConfigDocs(config: CssOutputConfig): string {
  const docs = [
    ...
    config.criticalCss?.enabled ? `- Strategy: ${config.criticalCss.strategy}` : '',
    config.criticalCss?.enabled ? `- Max Size: ${Math.round(config.criticalCss.maxSize / 1024)}KB` : '',
    '',
    '## Asset Hashing',
