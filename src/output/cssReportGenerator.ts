...
export class CssReportGenerator {
  ...
  private calculateBundleMetrics(results: {
    ...
    return results.bundles.map(bundle => {
      const bundleChunks = results.chunks.filter(chunk => 
        chunk.bundleId === bundle.id
      );
      ...
      return {
        ...
        selectorCount: bundle.rules?.reduce((sum, rule) => sum + (rule.selectors?.length || 0), 0) || 0,
        unusedRulesRemoved: Math.max(0, (bundle.rules?.length || 0) - bundleChunks.reduce((sum, chunk) => sum + (chunk.rules?.length || 0), 0)),
   ...
   */
  private calculateGlobalMetrics(bundleMetrics: BundlePerformanceMetrics[]): GlobalPerformanceMetrics {
    ...
    const totalCompressedSize = bundleMetrics.reduce((sum, bundle) => sum + bundle.compressedSize, 0);
    const totalChunkCount = bundleMetrics.reduce((sum, bundle) => sum + bundle.chunkCount, 0);
    const totalCriticalCssSize = bundleMetrics.reduce((sum, bundle) => sum + bundle.criticalCssSize, 0);
    const totalOptimizationTime = bundleMetrics.reduce((sum, bundle) => sum + bundle.optimizationTime, 0);
    
    const averageLoadTime = bundleMetrics.length > 0 
      ? bundleMetrics.reduce((sum, bundle) => sum + bundle.estimatedLoadTime, 0) / bundleMetrics.length
      : 0;
  ...
  private analyzeBudgetCompliance(metrics: GlobalPerformanceMetrics): {
    ...
    metrics.bundles.forEach(bundle => {
      if (bundle.compressedSize > budget.maxBundleSize) {
        violations.push({
          ...
          severity: bundle.compressedSize > budget.maxBundleSize * 1.5 ? 'error' : 'warning',
    ...
    if (metrics.totalCriticalCssSize > budget.maxCriticalCssSize) {
      violations.push({
        ...
        severity: metrics.totalCriticalCssSize > budget.maxCriticalCssSize * 1.3 ? 'error' : 'warning',
    ...
    if (metrics.totalCompressedSize > (budget.maxTotalSize || budget.maxBundleSize * 5)) {
      violations.push({
        type: 'total_size',
  ...
  private generateRecommendations(
    ...
    if (metrics.overallCompressionRatio > 0.8) {
      recommendations.push({
        ...
        description: 'Large bundles detected. Better chunking can improve caching and loading performance.',
    ...
    if (metrics.totalCriticalCssSize === 0) {
      recommendations.push({
        ...
        description: 'No critical CSS detected. Implementing critical CSS can significantly improve perceived performance.',
    ...
    const avgCacheEfficiency = metrics.bundles.reduce((sum, b) => sum + b.cacheEfficiency, 0) / metrics.bundles.length;
    ...
    if (metrics.averageLoadTime > 2000) {
      recommendations.push({
        ...
        description: 'High load times detected. Optimize delivery mechanism for better performance.',
  ...
  private determineChunkPriority(chunk: CssChunk): 'critical' | 'high' | 'medium' | 'low' {
    ...
    if (chunk.usagePattern?.score && chunk.usagePattern.score > 80) return 'high';
    if (chunk.usagePattern?.score && chunk.usagePattern.score > 50) return 'medium';
    return 'low';
  }
  ...
  private estimateContentStability(chunks: CssChunk[]): number {
    ...
    return chunks.reduce((avg, chunk) => {
   ...
   */
  async exportReport(report: CssPerformanceReport, format: 'json' | 'html' | 'markdown'): Promise<string> {
    switch (format) {
      case 'json':
...
<body>
    ...
    ${report.budgetAnalysis.violations.map(v => `
        <div class="violation">
...
${report.budgetAnalysis.violations.length > 0 ? '### Violations\n' + report.budgetAnalysis.violations.map(v => 
