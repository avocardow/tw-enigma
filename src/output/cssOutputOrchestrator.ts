...
export class CssOutputOrchestrator {
  ...
  private initializeComponents(): void {
    ...
    this.criticalCssExtractor = createCriticalCssExtractor(this.config.criticalCss);
  ...
  async orchestrate(
    ...
    for (const bundle of bundles) {
      try {
        ...
        warnings.push(`Failed to process bundle ${bundle.id}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  ...
  private async generateChunks(
    ...
    switch (strategy) {
      ...
      case 'usage':
        return this.chunker.chunkByUsage(bundle.content, {
          usageThreshold: this.config.chunking.usageThreshold,
          routes: bundle.routes || []
        });
        
      case 'route':
        return this.chunker.chunkByRoute(bundle.content, {
          routes: bundle.routes || options.routes || [],
          separateCommon: true
        });
        
      case 'component':
        return this.chunker.chunkByComponent(bundle.content, {
          components: bundle.components || [],
          separateVendor: this.config.chunking.separateVendor
        });
        
      case 'hybrid':
        return this.chunker.chunkHybrid(bundle.content, {
          maxSize: this.config.chunking.maxSize,
          usageThreshold: this.config.chunking.usageThreshold,
          routes: bundle.routes || options.routes || [],
          components: bundle.components || []
  ...
  private calculateGlobalStats(
    ...
    const overallCompressionRatio = totalSize > 0 
      ? (totalSize - totalCompressedSize) / totalSize 
      : 0;
  ...
  private calculatePerformanceMetrics(
    ...
    const loadingStrategy: 'single' | 'chunked' | 'modular' = 
      ...
      strategy === 'chunked' ? 'chunked' : 'modular';
  ...
  private validateBundleResult(result: CssOutputResult): string[] {
    ...
    for (const chunk of result.chunks) {
      ...
      if (chunkSize > 100 * 1024) { // 100KB
        warnings.push(`Chunk ${chunk.id} is large (${Math.round(chunkSize / 1024)}KB). Consider further splitting.`);
      }
    }
    
    // Check optimization effectiveness
    const optimizationRatio = result.stats.originalSize > 0 
      ? (result.stats.originalSize - result.stats.optimizedSize) / result.stats.originalSize
      : 0;
    
    if (optimizationRatio < 0.1) {
      warnings.push(`Bundle ${result.bundle.id} has low optimization ratio (${Math.round(optimizationRatio * 100)}%). Check CSS quality.`);
    }
    ...
    if (result.criticalCss) {
      ...
      if (criticalSize > this.config.criticalCss.maxSize) {
        warnings.push(`Critical CSS for ${result.bundle.id} exceeds recommended size (${Math.round(criticalSize / 1024)}KB).`);
      }
    }
 ...
 */
export function createProductionOrchestrator(overrides?: Partial<CssOutputConfig>): CssOutputOrchestrator {
 ...
 */
export function createDevelopmentOrchestrator(overrides?: Partial<CssOutputConfig>): CssOutputOrchestrator {
