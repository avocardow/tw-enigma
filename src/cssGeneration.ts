...
export const CssGenerationOptionsSchema = z.object({
  ...
  sortingStrategy: z.enum(['specificity', 'frequency', 'alphabetical', 'custom']).default('specificity'),
  commentLevel: z.enum(['none', 'minimal', 'detailed', 'verbose']).default('detailed'),
  selectorNaming: z.enum(['sequential', 'frequency-optimized', 'pretty', 'custom']).default('pretty'),
...
export class InvalidCssError extends CssGenerationError {
  constructor(message: string, public invalidCss: string, public reason?: string) {
...
export class ApplyDirectiveError extends CssGenerationError {
  constructor(message: string, public directive: string, public classes?: string[]) {
...
export class PatternClassificationError extends CssGenerationError {
  constructor(message: string, public className: string) {
...
export const CSS_PROPERTY_GROUPS = {
  ...
  FLEXBOX: ['flex', 'flex-direction', 'flex-wrap', 'justify-content', 'align-items', 'align-content'],
...
export const TAILWIND_DIRECTIVE_PATTERNS = {
  VARIANTS: /^(hover|focus|active|disabled|first|last|odd|even|visited|target|checked|indeterminate|default|required|valid|invalid|in-range|out-of-range|placeholder-shown|autofill|read-only):/,
...
export function generateCssRules(
  ...
  try {
    ...
    const filteredPatterns = patternsArray.filter(pattern => 
      pattern.totalFrequency >= actualOptions.minimumFrequency
    );
...
function analyzeSemanticGroups(classes: string[]): {
  ...
  for (const className of classes) {
    ...
    if (/^(flex|grid|block|inline|hidden|visible|relative|absolute|fixed|sticky)/.test(cleanClass)) {
...
export function validateApplyDirective(directive: ApplyDirective | string): Array<{ type: 'error' | 'warning'; message: string }> | boolean {
...
export function optimizeApplyDirective(directive: ApplyDirective, options: CssGenerationOptions): ApplyDirective {
  try {
...
function getVariantPriority(variantKey: string): number {
  ...
  for (const variant of variants) {
    switch (variant) {
      case 'dark': priority += 1; break;
      case 'sm': priority += 10; break;
      case 'md': priority += 20; break;
      case 'lg': priority += 30; break;
      case 'xl': priority += 40; break;
      case '2xl': priority += 50; break;
      case 'hover': priority += 100; break;
      case 'focus': priority += 110; break;
      case 'active': priority += 120; break;
      case 'disabled': priority += 130; break;
      case 'first': priority += 200; break;
      case 'last': priority += 210; break;
      case 'odd': priority += 220; break;
      case 'even': priority += 230; break;
      default: priority += 1000; break; // Unknown variants last
    }
  }
...
function optimizeForComponent(classes: string[]): string[] {
  ...
  const componentOrder = [
    'display', 'position', 'layout', 'sizing', 'spacing', 
...
function isRedundantInAtomic(className: string, allClasses: string[]): boolean {
  ...
  return samePropertyClasses.length > 1 && samePropertyClasses.indexOf(className) !== samePropertyClasses.length - 1;
}
...
export function classifyPattern(
  ...
  try {
    ...
    const type = determinePatternType(classes, frequency, complexity, coOccurrenceStrength, semanticGroups);
    
    // Calculate confidence based on classification certainty
    const confidence = calculateClassificationConfidence(type, classes, frequency, complexity, semanticGroups);
 ...
 */
function determinePrimarySemanticGroup(semanticGroups: ReturnType<typeof analyzeSemanticGroups>): string {
  const groupCounts = {
  ...
  const primaryGroup = Object.entries(groupCounts)
    .sort(([, a], [, b]) => b - a)[0][0];
...
function determinePatternType(
  ...
  if (classCount >= CSS_PATTERN_THRESHOLDS.COMPONENT_MIN_CLASSES &&
      complexity >= CSS_PATTERN_THRESHOLDS.COMPLEXITY_THRESHOLD_HIGH) {
    return 'component';
  }
  
  if (coOccurrenceStrength >= CSS_PATTERN_THRESHOLDS.CO_OCCURRENCE_STRONG &&
 ...
 */
function hasMultipleSemanticGroups(semanticGroups: ReturnType<typeof analyzeSemanticGroups>): boolean {
  const nonEmptyGroups = Object.values(semanticGroups)
...
export function sortCssRulesAdvanced(
  rules: CssRule[],
  options: CssGenerationOptions,
  criteria: Array<{ field: string; weight: number; order: 'asc' | 'desc' } | { type: string; weight: number; direction: 'asc' | 'desc' }>
): CssRule[] {
  try {
...
export function analyzePatternRelationships(
  ...
  try {
    ...
    for (const data of dataArray) {
      ...
      if (relatedClasses.length > 1) {
        clusters.push({
          ...
          cohesion: strongRelationships.reduce((sum, rel) => sum + rel.strength, 0) / strongRelationships.length
        });
    ...
    if (clusters.length > 0) {
      recommendations.push(`Found ${clusters.length} class clusters that could be optimized into components`);
    }
    
    const highFreqClasses = dataArray.filter(c => (c.totalFrequency || 0) > 50);
    if (highFreqClasses.length > 0) {
      recommendations.push(`${highFreqClasses.length} high-frequency classes could benefit from atomic CSS approach`);
    }
...
export function sortCssRules(
  ...
  try {
    ...
    const strategy = typeof strategyOrOptions === 'string' 
      ? strategyOrOptions 
...
function sortByFrequency(a: CssRule, b: CssRule): number {
  ...
  const typePriority = getPatternTypePriority(a.patternType) - getPatternTypePriority(b.patternType);
  if (typePriority !== 0) {
    return typePriority;
  }
...
function getPatternTypePriority(type: 'atomic' | 'utility' | 'component'): number {
  switch (type) {
    case 'atomic': return 1;    // Highest priority
    case 'utility': return 2;   // Medium priority
    case 'component': return 3; // Lowest priority
...
function validateRuleOrder(rules: CssRule[]): {
  ...
  const isFrequencyDescending = frequencies.every((freq, index) => 
    index === 0 || frequencies[index - 1] >= freq
  );
  ...
  try {
    ...
    if (actualCommentLevel === 'detailed' || actualCommentLevel === 'verbose') {
      ...
      for (const [type, count] of Object.entries(typeBreakdown)) {
        ...
        comments.push(` * ${type.charAt(0).toUpperCase() + type.slice(1)}: ${count} rules (${percentage}%)`);
      }
 ...
 */
function generateRuleSpecificComments(rule: CssRule, commentLevel: CssGenerationOptions['commentLevel']): string {
...
function generateOptimizationRecommendations(
  ...
  const highFrequencyRules = rules.filter(rule => 
    rule.frequency >= CSS_PATTERN_THRESHOLDS.HIGH_FREQUENCY_THRESHOLD
  );
  ...
  const lowFrequencyRules = rules.filter(rule => 
  ...
  const highSpecificityRules = rules.filter(rule => 
    calculateCssSpecificity(rule) > 50
  );
  ...
  if (statistics.generationTime > 5000) { // 5 seconds
    recommendations.push(
...
function generatePerformanceInsights(
  ...
  const avgCoOccurrence = rules.reduce((sum, rule) => sum + rule.coOccurrenceStrength, 0) / rules.length;
...
function calculatePerformanceScore(
  ...
  if (statistics.memoryUsage > 100 * 1024 * 1024) score -= 20; // 100MB
  else if (statistics.memoryUsage > 50 * 1024 * 1024) score -= 10; // 50MB
  
  // Deduct points for slow generation
  if (statistics.generationTime > 10000) score -= 15; // 10 seconds
...
export function integrateCssGeneration(
  ...
  try {
    ...
    const cssRules = sortedRules.map(rule => {
      const ruleComment = options.commentLevel === 'verbose' ? 
    ...
    const lowFrequencyRules = sortedRules.filter(rule => 
      rule.frequency <= CSS_PATTERN_THRESHOLDS.LOW_FREQUENCY_THRESHOLD
    );
    ...
    const highComplexityRules = sortedRules.filter(rule => 
      rule.complexity >= CSS_PATTERN_THRESHOLDS.COMPLEXITY_THRESHOLD_HIGH
    );
    ...
    const invalidApplyRules = sortedRules.filter(rule => 
    ...
    const sourceClasses = Array.from(new Set(
      patterns.flatMap(pattern => extractSourceClasses(pattern))
    ));
    ...
    throw new CssGenerationError(
      ...
      'INTEGRATION_FAILED',
      { analysisResult: analysisResultOrFrequencyMap, options: optionsOrNameOptions, cssOptions, error }
    );
  }
}
...
export function generateOptimizedCss(
  ...
  try {
    ...
    // Sort rules according to strategy
    const sortedRules = sortCssRules(rules, validatedOptions.sortingStrategy, validatedOptions.customSortFunction);
    ...
    const css = comments + sortedRules.map(rule => formatCssRule(rule, validatedOptions)).join('\n\n');
...
export function formatCssOutput(
  ...
  try {
    ...
    if (options.commentLevel === 'detailed' || options.commentLevel === 'verbose') {
      ...
      lines.push(`/* Compression Ratio: ${(result.statistics.compressionRatio * 100).toFixed(1)}% */`);
      lines.push(`/* Generation Time: ${result.statistics.generationTime.toFixed(2)}ms */`);
      lines.push(`/* Memory Usage: ${(result.statistics.memoryUsage / 1024 / 1024).toFixed(2)}MB */`);
...
export class EnhancedCSSGenerator {
  ...
  private async initializePostCSS(): Promise<void> {
    try {
      ...
      const { EnigmaPostCSSProcessor } = await import('./postcssIntegration.js');
      this.postcssProcessor = new EnigmaPostCSSProcessor(configManager.getProcessorConfig());
  ...
  async generateEnhancedCSS(
    ...
    try {
      ...
      if (this.postcssProcessor && options.enablePostCSS !== false) {
        const postcssResult = await this.processWithPostCSS(baseCSS.css, options);
